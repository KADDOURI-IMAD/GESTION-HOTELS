...
function send_mail($uemail, $token, $type){
    ...
    $email->setFrom("kaddouri.imad44@gmail.com", "STAR HOTEL");
...
if(isset($_POST['register'])) {
    ...
    $query = "INSERT INTO user_cred (fname, lname, email, address, phonenum, pincode, dob, profile, password, token) VALUES (?,?,?,?,?,?,?,?,?,?)";

    $values = [$data['fname'],$data['lname'],$data['email'],$data['address'],$data['phonenum'],$data['pincode'],$data['dob'],$img,$enc_pass,$token];
    ...
    if (mysqli_num_rows($u_exist) == 0) {
        ...
        if ($u_fetch['is_verified'] == 0) {
            ...
            if (!password_verify($data['pass'], $u_fetch['password'])) {
                ...
                $_SESSION['uName'] = $u_fetch['fname'] . ' ' . $u_fetch['lname'];
