    ...
    while($row = mysqli_fetch_assoc($res)){
        ...
        $data.="
            <tr class='align-middle'>
              ...
              <td>$status</td>
              <td>
              <button type='button' onclick='edit_details($row[id])' class='btn btn-primary shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#edit-rooms'>
              <i class='bi bi-pencil-square'></i></button>

              <button type='button' onclick=\"room_images($row[id],'$row[name]')\" class='btn btn-info shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#rooms-images'>
...
if(isset($_POST['get_rooms'])){
   ...
   $res2 = select("SELECT * FROM room_features WHERE room_id=?",[$frm_data['get_rooms']],'i');
   $res3 = select("SELECT * FROM room_facilities WHERE room_id=?",[$frm_data['get_rooms']],'i');
...
if(isset($_POST['get_room_images'])){
    ...
    while($row = mysqli_fetch_assoc($res)){
        ...
        if($row['thump']==1){
            ...
            $thumb_btn ="<button onclick='thump_image($row[sr_no],$row[room_id])' class='btn btn-secondary shadow-none'>
...
if(isset($_POST['thump_image'])){
    $frm_data = filteration($_POST);
    
    $pre_q = "UPDATE room_images SET thump =? WHERE room_id =?";
    $pre_v = [0,$frm_data['room_id']];
    $pre_res = update($pre_q,$pre_v,'ii');


    $pre_q = "UPDATE room_images SET thump =? WHERE sr_no =? AND  room_id =?";
    $pre_v = [1,$frm_data['image_id'],$frm_data['room_id']];
    $pre_res = update($pre_q,$pre_v,'iii');
